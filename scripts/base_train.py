...
parser.add_argument("--aspect-ratio", type=int, default=48, help="model_dim = depth * aspect_ratio")
...
# Evaluation
parser.add_argument("--log-every", type=int, default=10, help="print training progress every N steps (1 = every step)")
...
while True:
    ...
    if step % args.log_every == 0:
        ...
        print0(f"step {step:05d}/{num_iterations:05d} ({pct_done:.2f}%) | loss: {debiased_smooth_loss:.6f} | lrm: {lrm:.2f} | dt: {dt * 1000:.2f} | tok/sec: {tok_per_sec:,} | bf16_mfu: {mfu:.2f} | epoch: {epoch} | total time: {total_training_time/60:.2f}{eta_str}")
    if step % args.log_every == 0:
        log_data = {
            "step": step,
            "total_training_flops": flops_so_far,
